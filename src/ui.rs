...
fn draw_editor(f: &mut Frame, app: &App, area: Rect) {
    ...
    if app.focus == Focus::Editor {
        ...
        if lines_before_cursor >= app.editor_scroll && lines_before_cursor < app.editor_scroll + visible_height {
    ...
    if let Some(result) = &app.result {
        ...
        let rows = result
            ...
            .map(|(i, row)| {
                ...
                let color = if i % 2 == 0 { Color::White } else { Color::Cyan };
...
fn draw_status(f: &mut Frame, app: &App, area: Rect) {
    let status_text = vec![
        ...
        Line::from(vec![
            ...
            Span::raw(":Execute "),
            Span::styled("F6", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::raw(":Format "),
            Span::styled("F7", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
            Span::raw(":Load "),
            Span::styled("F8", Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD)),
            Span::raw(":Save "),
